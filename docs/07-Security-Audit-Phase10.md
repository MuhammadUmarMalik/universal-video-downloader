...
The browser suite now includes a mocked-Electron preload lifecycle contract covering public-source analysis, queue progress, completion refresh, simulated startup recovery, and a processing-state bridge rejection. This validates the React-to-IPC contract in the existing Vite/Playwright harness. It does not replace a packaged Electron application test that kills and relaunches the headless Rust child process against a real SQLite file; that native restart/crash test remains an environment-dependent release test.

## Remaining release controls

The full application security gate is complete for the current source-level implementation. The remaining controls are release-environment validations rather than unimplemented bypass or access behavior: native Windows/macOS permission and packaging checks, a packaged-FFmpeg validation matrix, a malformed-media fuzz/corpus run, and a real Electron process-kill/relaunch test with durable database state. These should remain explicit Phase 10 release checklist items and must not be represented as passing based only on the browser preview harness.

## Electron migration security boundary

Electron runs with context isolation, disabled Node integration, and a sandboxed preload. The renderer receives only an allowlisted command function and progress subscription callbacks. The main process validates command names before forwarding JSON lines to the headless Rust process. Rust retains authority over network access, filesystem writes, SQLite, FFmpeg, scheduler, queue state, startup recovery, and bandwidth enforcement. No renderer input can select an arbitrary executable, shell command, child process, filesystem path outside the existing Rust policy, or unrestricted IPC channel.
